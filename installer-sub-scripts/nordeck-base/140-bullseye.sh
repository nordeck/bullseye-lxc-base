...
cp -arp $MACHINE_HOST/usr/local/nordeck/cache/bullseye-apt-archives \
    $SHARED/cache/
...
apt-get $APT_PROXY \
    -o dir::cache::archives="/usr/local/nordeck/cache/bullseye-apt-archives/" \
...
# ssh
cp etc/ssh/sshd_config.d/nordeck.conf $ROOTFS/etc/ssh/sshd_config.d/
