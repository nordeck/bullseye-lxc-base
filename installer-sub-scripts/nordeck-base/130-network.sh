...
DNS_RECORD=$(grep 'address=/host/' etc/dnsmasq.d/nordeck-hosts | head -n1)
IP=${DNS_RECORD##*/}
...
# BACKUP & STATUS
# ------------------------------------------------------------------------------
OLD_FILES="/root/nordeck-old-files/$DATE"
mkdir -p $OLD_FILES
...
[[ -f /etc/dnsmasq.d/nordeck-hosts ]] && \
    cp /etc/dnsmasq.d/nordeck-hosts $OLD_FILES/
...
cp etc/dnsmasq.d/nordeck-hosts /etc/dnsmasq.d/
cp etc/dnsmasq.d/nordeck-resolv /etc/dnsmasq.d/
...
[[ -z "$(egrep '^source\s*interfaces.d/nordeck-bridge.cfg' /etc/network/interfaces || true)" ]] && \
[[ -z "$(egrep '^source\s*/etc/network/interfaces.d/nordeck-bridge.cfg' /etc/network/interfaces || true)" ]] && \
echo -e "\nsource /etc/network/interfaces.d/nordeck-bridge.cfg" >> /etc/network/interfaces
...
if [[ -f "/etc/network/cloud-interfaces-template" ]]; then
    ...
    [[ -z "$(egrep '^source\s*interfaces.d/nordeck-bridge.cfg' /etc/network/cloud-interfaces-template || true)" ]] && \
    [[ -z "$(egrep '^source\s*/etc/network/interfaces.d/nordeck-bridge.cfg' /etc/network/cloud-interfaces-template || true)" ]] && \
    echo -e "\nsource /etc/network/interfaces.d/nordeck-bridge.cfg" >> /etc/network/cloud-interfaces-template
fi

# IP forwarding
cp etc/sysctl.d/nordeck-ip-forward.conf /etc/sysctl.d/
sysctl -p /etc/sysctl.d/nordeck-ip-forward.conf || true
...
cp etc/network/interfaces.d/nordeck-bridge.cfg /etc/network/interfaces.d/
sed -i "s/___MAC_ADDRESS___/${MAC_ADDRESS}/g" \
    /etc/network/interfaces.d/nordeck-bridge.cfg
sed -i "s/___BRIDGE___/${BRIDGE}/g" /etc/network/interfaces.d/nordeck-bridge.cfg
cp etc/dnsmasq.d/nordeck-interface /etc/dnsmasq.d/
sed -i "s/___BRIDGE___/${BRIDGE}/g" /etc/dnsmasq.d/nordeck-interface

ifup -i /etc/network/interfaces.d/nordeck-bridge.cfg dummy0
ifup -i /etc/network/interfaces.d/nordeck-bridge.cfg $BRIDGE
...
if [[ "$RECREATE_CUSTOM_NFTABLES" = true ]]; then
    nft delete table inet nordeck-filter 2>/dev/null || true
    nft delete table ip nordeck-nat 2>/dev/null || true
fi

# table: nordeck-filter
# chains: input, forward, output
# rules: drop from the public interface to the private internal network
nft add table inet nordeck-filter
nft add chain inet nordeck-filter \
    input { type filter hook input priority 0 \; }
nft add chain inet nordeck-filter \
    forward { type filter hook forward priority 0 \; }
nft add chain inet nordeck-filter \
    output { type filter hook output priority 0 \; }
[[ -z "$(nft list chain inet nordeck-filter output | \
ack 'ip daddr 172.22.22.0/24 drop')" ]] && \
    nft add rule inet nordeck-filter output \
    iif $PUBLIC_INTERFACE ip daddr 172.22.22.0/24 drop

# table: nordeck-nat
# chains: prerouting, postrouting, output, input
# rules: masquerade
nft add table ip nordeck-nat
nft add chain ip nordeck-nat prerouting \
    { type nat hook prerouting priority 0 \; }
nft add chain ip nordeck-nat postrouting \
    { type nat hook postrouting priority 100 \; }
nft add chain ip nordeck-nat output \
    { type nat hook output priority 0 \; }
nft add chain ip nordeck-nat input \
    { type nat hook input priority 0 \; }
[[ -z "$(nft list chain ip nordeck-nat postrouting | \
ack 'ip saddr 172.22.22.0/24 masquerade')" ]] && \
    nft add rule ip nordeck-nat postrouting \
...
# rules: tcp dnat
nft add map ip nordeck-nat tcp2ip \
    { type inet_service : ipv4_addr \; }
nft add map ip nordeck-nat tcp2port \
    { type inet_service : inet_service \; }
[[ -z "$(nft list chain ip nordeck-nat prerouting | \
ack 'tcp dport map @tcp2ip:tcp dport map @tcp2port')" ]] && \
    nft add rule ip nordeck-nat prerouting \
...
# rules: udp dnat
nft add map ip nordeck-nat udp2ip \
    { type inet_service : ipv4_addr \; }
nft add map ip nordeck-nat udp2port \
    { type inet_service : inet_service \; }
[[ -z "$(nft list chain ip nordeck-nat prerouting | \
